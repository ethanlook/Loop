...
final class DeviceDataManager: CarbStoreDelegate, DoseStoreDelegate, TransmitterDelegate, ReceiverDelegate {
    ...
    func doseStore(doseStore: DoseStore, hasEventsNeedingUpload pumpEvents: [PersistedPumpEvent], fromPumpID pumpID: String, withCompletion completionHandler: (uploadedObjects: [NSManagedObjectID]) -> Void) {
        guard let pumpModel = pumpState?.pumpModel else {
            completionHandler(uploadedObjects: pumpEvents.map({ $0.objectID }))
            return
        }
        
        if (remoteDataManager.nightscoutUploader == nil && remoteDataManager.tidepoolClient == nil) {
            completionHandler(uploadedObjects: pumpEvents.map({ $0.objectID }))
            return
        }
        
        let nsUploader = remoteDataManager.nightscoutUploader
        let tidepoolClient = remoteDataManager.tidepoolClient
        ...
        nsUploader?.upload(timestampedPumpEvents, forSource: "loop://\(UIDevice.currentDevice().name)", from: pumpModel) { (error) in
            if let error = error {
        ...
        let tidepoolEvents = TidepoolService.translatePumpEvents(timestampedPumpEvents)
        tidepoolClient?.uploadData(tidepoolEvents) { (success, error) in
            if (success) {
                NSLog("Successfully uploaded data to Tidepool: \(self.remoteDataManager.tidepoolService.username!)")
            } else {
                self.logger.addError(error!, fromSource: "TidepoolKit")
            }
        }
    }
